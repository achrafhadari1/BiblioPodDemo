...
import { useCustomPaginatedManager } from "../EpubReaderComponents/CustomPaginatedManager";
...
function EpubReader() {
  ...
  const {
    ...
    navigateToSection: scrollNavigateToSection,
    navigateToHref: scrollNavigateToHref,
    next: scrollNext,
    prev: scrollPrev,
    applyTheme: scrollApplyTheme,
    applyFontSettings: scrollApplyFontSettings,
    setSavedProgress: scrollSetSavedProgress,
    restoreProgress: scrollRestoreProgress,
  ...
  // Custom paginated manager hook - only used when readingMode is "paginated"
  const {
    manager: paginatedManager,
    currentLocation: paginatedCurrentLocation,
    readingProgress: paginatedReadingProgress,
    isInitialized: paginatedManagerInitialized,
    currentPage: paginatedCurrentPage,
    totalPages: paginatedTotalPages,
    navigateToSection: paginatedNavigateToSection,
    navigateToHref: paginatedNavigateToHref,
    goToPage: paginatedGoToPage,
    next: paginatedNext,
    prev: paginatedPrev,
    applyTheme: paginatedApplyTheme,
    applyFontSettings: paginatedApplyFontSettings,
    setSavedProgress: paginatedSetSavedProgress,
    restoreProgress: paginatedRestoreProgress,
  } = useCustomPaginatedManager(
  ...
  // Update current chapter when paginated manager location changes
  useEffect(() => {
    if (readingMode === "paginated" && paginatedCurrentLocation && book) {
      console.log(
        "[CHAPTER] Updating chapter from paginated manager location:",
        paginatedCurrentLocation
      );
      updateCurrentChapter(paginatedCurrentLocation, book);
    }
  }, [paginatedCurrentLocation, book, readingMode]);
  ...
  // Update current percentage when paginated manager progress changes
  useEffect(() => {
    if (
      readingMode === "paginated" &&
      typeof paginatedReadingProgress === "number"
    ) {
      console.log(
        "[PROGRESS] Updating percentage from paginated manager:",
        paginatedReadingProgress
      );
      setCurrentPercentage(Math.round(paginatedReadingProgress * 100));
    }
  }, [paginatedReadingProgress, readingMode]);

  // Update page info when paginated manager updates
  useEffect(() => {
    if (
      readingMode === "paginated" &&
      ...
      console.log(
        "[PAGE] Updating page info from paginated manager:",
        paginatedCurrentPage,
        "/",
        paginatedTotalPages
      );
      setCurrentPage(paginatedCurrentPage);
      setTotalPages(paginatedTotalPages);
    }
  }, [paginatedCurrentPage, paginatedTotalPages, readingMode]);

  // Auto-save progress when location changes (debounced)
  ...
  useEffect(() => {
    ...
    if (currentLocation && bookData) {
      ...
      autoSaveTimeoutRef.current = setTimeout(() => {
        console.log(`[PROGRESS] Auto-saving progress in ${readingMode} mode`);
  ...
  }, [scrollCurrentLocation, paginatedCurrentLocation, readingMode, bookData]);
  ...
  const nextBtn = useCallback(() => {
    if (readingMode === "scrolled") {
      ...
      // Use custom paginated manager for paginated mode
      if (paginatedNext) {
        paginatedNext();
      }
    }
  }, [readingMode, scrollNext, paginatedNext]);
  ...
  }, [readingMode, scrollPrev, paginatedPrev]);
  ...
  useEffect(() => {
    if (readingMode === "scrolled") {
      ...
      // Use custom paginated manager for theme
      if (paginatedApplyTheme && preferencesLoaded) {
        console.log(
          "[THEME] Applying theme via paginated manager:",
          isDarkTheme
        );
        paginatedApplyTheme(isDarkTheme);
      }
    } else if (rendition && rendition.themes && preferencesLoaded) {
      // Use regular rendition for theme (fallback)
      if (isDarkTheme) {
        if (isDarkClicked > 0) {
  ...
  const applyFontSettingsImmediate = (
    ...
    if (readingMode === "scrolled") {
      ...
      // Use custom paginated manager for font settings
      if (paginatedApplyFontSettings) {
        console.log("[FONT] Applying font settings via paginated manager:", {
          fontSize,
          fontFamily,
          paginatedApplyFontSettings: !!paginatedApplyFontSettings,
        });
        paginatedApplyFontSettings(fontSize, fontFamily);
        console.log("[FONT] Paginated manager font settings applied");
      } else {
        console.log("[FONT] Paginated manager not available for font settings");
      }
      return;
    }
    ...
    try {
      ...
      if (readingMode === "scrolled" && scrollCurrentLocation) {
        ...
        newCFI =
          ...
          currentCFI;
        newPercentage = Math.round(scrollReadingProgress * 100);
      ...
      } else if (readingMode === "paginated" && paginatedCurrentLocation) {
        // Use paginated manager location
        console.log("[PROGRESS] Saving progress from paginated manager");
        newCFI =
          paginatedCurrentLocation.start?.cfi ||
          paginatedCurrentLocation.cfi ||
          currentCFI;
        newPercentage = Math.round(paginatedReadingProgress * 100);

        // If we still don't have a CFI, try to generate one from the current location
        if (!newCFI && paginatedCurrentLocation.href && book) {
          try {
            // Generate a basic CFI for the start of the section
            const sectionIndex = paginatedCurrentLocation.index;
            if (sectionIndex !== undefined && sectionIndex >= 0) {
              // Create a basic CFI pointing to the start of the section (consistent with CustomPaginatedManager)
              newCFI = `/6/${(sectionIndex + 1) * 2}[id${sectionIndex}]!/0`;
              console.log(
                "[PROGRESS] Generated CFI from section index:",
                newCFI
  ...
  const updateReadingMode = async (newMode) => {
    ...
    // Get current location from the active manager
